...
public class MobAttackHelper {
    ...
    public static float getAttackCooldownTicksCapped(Mob mob) {
        return Math.max(((MobDuck)mob).bettermobcombat$getCurrentItemAttackStrengthDelay(), (float)BetterCombat.config.attack_interval_cap);
    }
    ...
    private static AttackSelection selectAttack(int comboCount, WeaponAttributes attributes, LivingEntity mob, boolean isOffHandAttack) {
        ...
        attacks = (WeaponAttributes.Attack[])Arrays.stream(attacks).filter((attack) -> {
            return attack.conditions() == null || attack.conditions().length == 0 || evaluateConditions(attack.conditions(), mob, isOffHandAttack);
    ...
    private static boolean evaluateConditions(WeaponAttributes.Condition[] conditions, LivingEntity mob, boolean isOffHandAttack) {
        return Arrays.stream(conditions).allMatch((condition) -> {
            return evaluateCondition(condition, mob, isOffHandAttack);
        });
    }
